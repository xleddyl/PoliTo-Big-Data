...
* @input: textual file containng pairs of users (friends)
* @output: textual file containing one line for each user alongside with their friends list
*/
...
class ReducerEX extends Reducer<Text, Text, Text, NullWritable> {
   protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
      ...
      for(Text value : values) {
         friends = friends.concat(value + " ");
      }
      context.write(new Text(new String(key.toString() + ": " + friends)), NullWritable.get());
   }
}
